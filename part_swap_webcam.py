...
def display_video(swap_index, source_image, reconstruction_module, segmentation_module, face_parser=None,
               hard=False, use_source_segmentation=False, cpu=False):
    assert type(swap_index) == list
    with torch.no_grad():
        ...
        while True:
            ...
            new_frame = resize(frame, (256, 256))[..., :3]
            target_frame = torch.tensor(new_frame[np.newaxis].astype(np.float32)).permute(0, 3, 1, 2)
            ...
            output_frame = img_as_ubyte(np.transpose(out['prediction'].data.cpu().numpy(), [0, 2, 3, 1])[0])

            cv2.imshow('Result', output_frame)
            key = cv2.waitKey(10)  # ESC to quit
            if key == 27:
                break
...
if __name__ == "__main__":
    ...
    display_video(opt.swap_index, source_image, reconstruction_module, segmentation_module,
                  face_parser, hard=opt.hard, use_source_segmentation=opt.use_source_segmentation,
                  cpu=opt.cpu)